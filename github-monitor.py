...
def render_status(info: BuildStatus, row, draw):
    ...
    draw.bitmap([98, y_offset], pr_icon, fill='DarkGrey')
    draw.bitmap([98, y_offset + 12], branch_icon, fill='DarkGrey')

    draw.text([112, y_offset - 2], "{0}".format(info.pr_count), font=small_font, fill='White')
    draw.text([112, y_offset + 10], "{0}".format(info.branch_count), font=small_font, fill='White')
